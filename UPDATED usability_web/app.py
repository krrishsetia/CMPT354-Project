...
from control.division_c import division_query_data
from control.delete_c import get_all_robots_for_delete, preview_robot_delete, perform_delete_robot
from control.update_c import get_all_robots_for_update, perform_update_robot_name
...
FUNCTIONALITIES = [
    ...
    },
    {
        "slug": "division-query",
        "number": "F11",
        "title": "Robots that have all required parts",
        "category": "Division query",
        "description": "Presentation-ready page for the division-query requirement. Enter a set of part IDs and show robots that contain all of them across their sub-assemblies.",
        "demo_note": "Use this page for the division-query rubric item by showing that a robot is returned only if it matches every required part in the selected set.",
        "dummy_fields": ["Required Part IDs (comma-separated)"],
        "dummy_rows": [
            ["Required parts", "101, 118, 205"],
            ["Matching robot", "Arm Robot", "Contains all three required parts"],
        ],
    },
]
...
def functionality_detail(slug: str):
    ...
    #F9 defaults
    #uses f6 filtering: subtype and mech_type
    low_parts = []
    filtered_parts = []
    usage_rows = []
    grouped_rows = []
    ...
    # F2 Update defaults
    all_robots_for_update = []
    update_old_name = None
    update_new_name = None
    update_error = None
    update_success = False

    # F3 Delete defaults
    all_robots_for_delete = []
    delete_preview = None
    delete_error = None
    ...
    # F11 defaults
    required_parts_raw = request.args.get("required_parts", "")
    division_results = []
    division_error = None
    ...
    elif slug == "division-query":
        required_part_ids = [part.strip() for part in required_parts_raw.split(",") if part.strip()]
        division_results, division_error = division_query_data(required_part_ids)

    if slug == "aggregation-reports":
        subtype, mech_type, part_id, low_parts, filtered_parts, usage_rows, grouped_rows, f9_error = f9_parts_page_data()
    ...
    return render_template(
        ...
        part_id=part_id,
        f9_error=f9_error,
        all_robots_for_update=all_robots_for_update,
        update_old_name=update_old_name,
        update_new_name=update_new_name,
        update_error=update_error,
        update_success=update_success,
        all_robots_for_delete=all_robots_for_delete,
        delete_preview=delete_preview,
        delete_error=delete_error,
        delete_success=delete_success,
        delete_audit=delete_audit,
        required_parts_raw=required_parts_raw,
